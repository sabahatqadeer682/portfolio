...
export default function Contact() {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const mailto = `mailto:sabahatqadeerbhati@gmail.com?subject=${encodeURIComponent(
      subject
    )}&body=${encodeURIComponent(
      `Name: ${name}\nEmail: ${email}\n\n${message}`
    )}`;
    window.location.href = mailto;
    setStatus("✓ Opening your email client… your message is ready to send!");
    f.reset();
        ...
        <div className="grid items-start gap-6 lg:grid-cols-[1fr_1.3fr]">
          ...
          <Reveal>
            <div className="term-window">
              ...
              <div className="p-6 font-mono text-[13px]">
                ...
                <pre className="mt-3 overflow-x-auto text-slate-400">
          ...
          <Reveal delay={100}>
            <div className="term-window">
              ...
              <form onSubmit={onSubmit} className="p-6 font-mono">
                ...
                <button
                  type="submit"
                  className="group mt-5 inline-flex w-full items-center justify-center gap-2 rounded-md border border-cyan-500/50 bg-cyan-500/10 px-5 py-3 text-[13px] font-semibold text-cyan-300 transition-all hover:bg-cyan-500/20"
                >
                ...
                {status && (
                  <div className="mt-4 rounded-md border border-emerald-500/30 bg-emerald-500/10 p-3 text-center text-[12px] text-emerald-400">
                    {status}
                  </div>
                )}
              </form>
    ...
    <div>
      ...
      {textarea ? (
        <textarea
          ...
          className="block w-full rounded-md border border-slate-800 bg-slate-950/70 px-3 py-2 font-mono text-[13px] text-emerald-300 placeholder-slate-700 outline-none transition-colors focus:border-cyan-500/60 focus:ring-2 focus:ring-cyan-500/25"
        />
      ) : (
        <input
          id={name}
